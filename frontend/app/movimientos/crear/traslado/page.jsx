...
export default function TrasladoPage() {
    ...
    const [materiales, setMateriales] = useState([{ material_id: "", cantidad: "" }]);
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || "Error registrando traslado");
  
        MySwal.fire({ title: "Traslado registrado", icon: "success", timer: 1200, showConfirmButton: false });
